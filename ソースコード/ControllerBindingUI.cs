...
public class ControllerBindingUI : MonoBehaviour
{
                ...
                foreach (var device in InputSystem.devices)
                {
                    ...
                    if (pressedBtn != null)
                    {
                        if (!IsKey(pressedBtn, Key.F1) && !IsKey(pressedBtn, Key.F2))
                        {
                            ...
                            // 保存：次のシーンで再バインドに使う
                            PlayerPrefs.SetString("P1Device", TurnManager.Instance?.player1Input.device?.displayName ?? "");
                            PlayerPrefs.SetString("P2Device", TurnManager.Instance?.player2Input.device?.displayName ?? "");
                            PlayerPrefs.Save();

                            // 新しいシーンがロードされたら再バインドを実行
                            SceneManager.sceneLoaded += OnCharacterSceneLoaded;
                            SceneManager.LoadScene("CharacterSetupScene");
                            return;
                        }
                    }
    ...
    private void OnCharacterSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene.name != "CharacterSetupScene") return;
        SceneManager.sceneLoaded -= OnCharacterSceneLoaded;

        var runner = new GameObject("SceneRebindRunner").AddComponent<SceneRebindRunner>();
...
public class SceneRebindRunner : MonoBehaviour
{
    public void Run() => StartCoroutine(RebindDevicesWhenReady());

    private IEnumerator RebindDevicesWhenReady()
    {
